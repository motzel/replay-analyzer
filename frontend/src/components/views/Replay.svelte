    ...
    {:else if replay}
        <section class="replay">
            ...
            {#each [
                ...
                {name: 'TD', tooltip: 'Time dependence', key: 'timeDependence', digits: 2, type: 'decimal', reverse: true},
            ] as stat}
                <div>
                    <StatsBadge variant="default" label={`${stat.name} left`} tooltip={stat.tooltip ?? null} digits={stat.digits}
                                type={stat.type} reverse={stat?.reverse ?? false}
                                stat={data?.stats?.left?.[stat.key]}
                    />

                    <StatsBadge variant="neutral" label={`${stat.name} total`} tooltip={stat.tooltip ?? null} digits={stat.digits}
                                type={stat.type} reverse={stat?.reverse ?? false}
                                stat={data?.stats?.total?.[stat.key]}
                    />

                    <StatsBadge variant="default" label={`${stat.name} right`} tooltip={stat.tooltip ?? null} digits={stat.digits}
                                type={stat.type} reverse={stat?.reverse ?? false}
            ...
            TODO: directionGrid, positionGrid, positionAndDirectionGrid
